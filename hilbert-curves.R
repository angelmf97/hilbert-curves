...
plot.results <- function(generator, xfrac = 2, yfrac = 2, n){
...
main <- function(order, corner = "bottomleft", orientation = "clockwise",
  ...
  for(n in 1:order){
    ...
    final_generator <- reorient(generator, corner = corner, orientation = orientation)
    if(animation==TRUE){
      plot.results(final_generator,xfrac, yfrac, n = n)
      Sys.sleep(sleeptime)
    }
    if(animation==FALSE){plot.results(final_generator, xfrac, yfrac, n = order)}
  }
}
